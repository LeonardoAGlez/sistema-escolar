...
export class EventosScreenComponent implements OnInit {
      ...
      (response) => {
        ...
        if (this.lista_eventos.length > 0) {
          this.lista_eventos.forEach((evento) => {
            ...
            // Formatear horario
            evento.horario_display = `${evento.hora_inicio?.slice(
              0,
              5
            ...
            if (evento.responsable_info) {
              evento.responsable_display =
                evento.responsable_info.nombre_completo;
            } else {
  ...
  public filtrar(event: Event): void {
    ...
    this.dataSource.filterPredicate = (data: DatosEvento, filter: string) => {
      return data.nombre_evento.toLowerCase().includes(filter);
    };
